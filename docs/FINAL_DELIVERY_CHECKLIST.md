...
- 方案 B / Open Design：当前已完成轻量智能 IDE 壳层；若继续做文件预览、diff、浏览器内编辑器或更完整视觉系统，应引入 Open Design，并在 `SPEC.md` 中补充设计系统与 skill。

## 建议下一步

1. 完成方案 A 验证后，经用户确认再提交本轮修改。
2. 用户手动 push 后，在 GitHub 上确认 CI 运行状态。
3. 若继续方案 B，下一步实现文件预览、diff/结果面板，并用 Open Design 补充更完整视觉系统。
